...
import com.Bibibi.entity.dto.VideoInfoEsDto;
...
public class EsSearchComponent {
    ...
    @Resource
    private UserInfoMappers userInfoMapper;
    ...
    public void saveDoc(VideoInfo videoInfo) throws BusinessException {
        try {
            ...
            log.error("保存到es失败", e);
            throw new BusinessException("保存到es失败", e);
        }
    }
        ...
        try {
            ...
            for (Field field : fields) {
                ...
                if (object != null && object instanceof String && StringTools.isEmpty(object.toString()) || object != null && !(object instanceof String)) {
                    dateMap.put(field.getName(), object);
                }
            }
            if (dateMap.isEmpty()) {
                return;
    ...
    public void delDoc(String videoId) throws BusinessException {
    ...
    public PaginationResultVO<VideoInfo> search(Boolean highlight, String keyword, Integer orderType, Integer pageNo, Integer pageSize) throws BusinessException {
        ...
        try {
            ...
            SearchResponse serverResponse = restHighLevelClient.search(searchRequest, RequestOptions.DEFAULT);

            SearchHits hits = serverResponse.getHits();
            ...
            videoInfoList.forEach(item -> {
                ...
                item.setNickName(userInfo == null ? "" : userInfoMap.get(item.getUserId()).getNickName());
            });

            SimplePage page = new SimplePage(pageNo, totalCount, pageSize);
            PaginationResultVO<VideoInfo> resultVO = new PaginationResultVO<>(totalCount, page.getPageNo(), page.getPageTotal(), videoInfoList);
